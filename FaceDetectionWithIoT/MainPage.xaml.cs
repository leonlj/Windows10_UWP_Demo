...
namespace CameraFaceDetection
{
    ...
    {
        private readonly IFaceServiceClient faceServiceClient = new FaceServiceClient("9ac427b7e9924fee8f8d1ab6ae11bead");
        private string facesAttr = "";
        ...
        private static readonly Guid RotationKey = new Guid("C380465D-2271-428C-9B83-ECEA3B4A85CD");

        string uploadAddress = "http://iotfacedetection.azurewebsites.net/api/FaceDetection?DeviceId=3d16334ca04a412388bae34d0dbf300f";
        ...
        private async Task SendPhotoAsync()
        {
            ...
            serviceBusNamespace: "freh-ns",
            eventHubName: "freh",
            keyName: "manager",
            key: "OrUdceuFytN8lL88ck8j3WEbH27T0r96pcnJCOXDWQ8=",
            ...
            try
            {
                ...
                {
                    stream.Seek(0);
                    //faceRects = await UploadAndDetectFaces(WindowsRuntimeStreamExtensions.AsStreamForRead(stream.GetInputStreamAt(0)));


                    faceRects = await UploadAndDetectFaces(stream.AsStream());

                    number = faceRects.Length;
        ...
        private async Task<FaceRectangle[]> UploadAndDetectFaces(Stream imageFilePath)
        {

            try
            {
                using (Stream imageFileStream = imageFilePath)
                {
                    Face[] faces = await faceServiceClient.DetectAsync(imageFileStream, false, true, new FaceAttributeType[] { FaceAttributeType.Gender, FaceAttributeType.Age, FaceAttributeType.Smile, FaceAttributeType.Glasses });
                    var faceRects = faces.Select(face => face.FaceRectangle);
                    facesAttr += "Detected face amount : " + faces.Length;
                    string emotion;
                    for (int i = 0; i < faces.Length; i++)
                    {
                        int number = i + 1;
                        facesAttr += "\n\nNo. " + number;
                        facesAttr += "\nGender: " + faces[i].FaceAttributes.Gender;
                        facesAttr += "\nAge: " + faces[i].FaceAttributes.Age;
                        //facesAttr += "\nHeadPose: \n[Yaw]:" + faces[i].FaceAttributes.HeadPose.Yaw + "\n[Roll]:" + faces[i].FaceAttributes.HeadPose.Roll + "\n[Pitch]:" + faces[i].FaceAttributes.HeadPose.Pitch;
                        emotion = toEmotion(faces[i].FaceAttributes.Smile);
                        facesAttr += "\nSmile: " + emotion;

                        SensorDataUpload(faces[i].FaceAttributes, emotion);
                    }


                    await this.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, () => { this.infoshowText.Text = facesAttr; });
                    facesAttr = "";
        ...
        private string toEmotion(double smile)
        {

            if (smile < 0.2) return "anger";
            if (smile < 0.4) return "sad";
            if (smile < 0.6) return "neutral";
            if (smile < 0.8) return "smile";
            return "happy";

        }

        ConnectTheDotsHelper ctdHelper;
        private void SensorDataUpload(FaceAttributes FaceAttributes, string emotion)
        {
            ConnectTheDotsSensor sensor = ctdHelper.sensors[0];
            sensor.guid = Guid.NewGuid().ToString();
            sensor.accesstime = DateTime.UtcNow.ToString();
            sensor.age = FaceAttributes.Age.ToString("0");
            sensor.emotion = emotion; //FaceAttributes.Smile.ToString();
            sensor.location = "Beijing_LSH";
            sensor.deviceid = "99";
            sensor.gender = FaceAttributes.Gender;
